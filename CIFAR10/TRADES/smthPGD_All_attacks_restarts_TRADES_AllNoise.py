...
from smooth_attack_pgdL1Maini_restarts_utils_TRADES_AllNoise import eval_adv_test_whitebox

from models.resnet import *  # Added by Ameya ..... 
...
parser.add_argument('--step-size-fact', default=0.1, type=float, help='step size as a fraction of epsilon')
...
def main():
    ...
    accuracy_list_file = open(res_dir_dim+"TRADES-Noise"+str(args.noise_dist)+"_NoisePW"+str(math.floor(args.noise_pw))+"Basis-"+str(args.noise_shp_basis)+"_SmoothPGDRestarts_"+args.attack_type+"_Attack_Steps"+str(args.num_steps)+"_8xRange_CorrInit.txt","a")
    ...
    for iter_ind in range(args.acc_sd_iters):
        ...
        torch.save(dict_to_save, vec_dir_dim+"TRADES-Noise"+str(args.noise_dist)+"_NoisePW"+str(math.floor(args.noise_pw))+"Basis-"+str(args.noise_shp_basis)+"_SmoothPGDRestarts_"+args.attack_type+"Attack_Steps"+str(args.num_steps)+"_Epsilon"+str(args.epsilon)+"_StpSz"+str(args.step_size_fact)+"_ErrVecs.pt")

        # pert_dict_to_save = {'attack_type':args.attack_type, 'attack_eps':args.epsilon, 'attack_steps':args.num_steps, 'stored_pert_vecs':stored_pert_vecs}
        # torch.save(pert_dict_to_save, vec_dir_dim+"TRADES-Noise"+str(args.noise_dist)+"_NoisePW"+str(math.floor(args.noise_pw))+"Basis-"+str(args.noise_shp_basis)+"_SmoothPGD_"+args.attack_type+"Attack_Steps"+str(args.num_steps)+"_PertbVecs.pt")
        ...
        print(robust_acc)
        file_str_x = f"{args.epsilon:5.5}\t{natural_acc:10.8}\t{robust_acc:10.8}\t{args.eval_epoch:5}\t{args.num_restarts:5}\t{args.num_tests:5}\t{args.step_size_fact:10.4}\n"
