...
def eval_adv_test_whitebox(model, test_loader, args):
    ...
    store_vec_fname = args.store_vec_fname

    if args.is_resume:

        print("attempting to resume.....")

        loaded_dict = torch.load(store_vec_fname)
        stored_err_rob_vec = loaded_dict['stored_err_rob_vec']
        stored_err_nat_vec = loaded_dict['stored_err_nat_vec']
        loaded_batch_ind = loaded_dict['batch_ind']

        print("Found the file. Loaded batch_ind:{}".format(loaded_batch_ind))

    else:
        print("Starting from the beginning...")
        loaded_batch_ind = args.start_batch_ind - 1
    ...
    if args.attack_type == 'l1':
        ...
        for data, target in test_loader:
            ...
            if batch_ind > loaded_batch_ind:
                ...
                if 'stored_err_nat_vec' in locals():
                ...
                print("Size of stored_err_nat_vec:")
                print(stored_err_nat_vec.size(0))
                print("Current natural acc:{}".format(1 - (stored_err_nat_vec!=0).float().sum()/float(torch.numel(stored_err_nat_vec))))
                print("Current robust acc:{}".format(1 - (stored_err_rob_vec!=0).float().sum()/float(torch.numel(stored_err_rob_vec))))  

                if ((batch_ind) % 10) == 0:
                    dict_to_save = {'batch_ind':batch_ind, 'attack_type':args.attack_type, 'attack_eps':args.epsilon, 'attack_steps':args.num_steps, 'stored_err_nat_vec':stored_err_nat_vec, 'stored_err_rob_vec':stored_err_rob_vec}
                    torch.save(dict_to_save, store_vec_fname)

                if batch_ind == args.end_batch_ind: 
                    dict_to_save = {'batch_ind':batch_ind, 'attack_type':args.attack_type, 'attack_eps':args.epsilon, 'attack_steps':args.num_steps, 'stored_err_nat_vec':stored_err_nat_vec, 'stored_err_rob_vec':stored_err_rob_vec}
                    torch.save(dict_to_save, store_vec_fname)
                    print("Reached end_batch_ind.... Ending the evaluation here....")
                    break
    ...
    natural_acc_fraction = 1 - natural_err_total.item()/float(torch.numel(stored_err_nat_vec))
    robust_acc_fraction = 1 - robust_err_total.item()/float(torch.numel(stored_err_rob_vec))
    print('natural_acc_fraction (test): ', natural_acc_fraction)
    print('robust_acc_fraction (test): ', robust_acc_fraction)

    return natural_acc_fraction, robust_acc_fraction, stored_err_nat_vec.cpu(), stored_err_rob_vec.cpu() #, stored_pert_vecs.cpu()


def pgd_l1_topk(model, X,y, args, alpha = 0.25, k = 200, restarts = 1, version = 0):
    ...
    for r in range(restarts):
        ...
        with torch.no_grad():
            for step in range(smth_avg_steps):
                ...
                out_pgd = model(X.data+delta.data)
    ...
    with torch.no_grad():
        for step in range(smth_avg_steps):
            ...
            out = model(X.data)
            out_pgd = model(X.data+max_delta.data)
...
def _pgd_l2_whitebox(model,X,y, args):
    ...
    step_size = 0.1*epsilon #*8/num_steps
    smth_avg_steps = args.smth_avg_steps
    num_avg_steps = args.grad_avg_steps
    restarts = args.num_restarts
